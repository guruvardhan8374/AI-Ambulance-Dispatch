...
export const HospitalLayout = () => {
  ...
  const navItems = [
    { to: "/hospital/dashboard", label: "ER Dashboard", icon: LayoutDashboard, end: true },
    { to: "/hospital/incoming", label: "Incoming Ambulances", icon: Truck },
    { to: "/hospital/capacity", label: "Capacity Management", icon: Activity },
    { to: "/hospital/status", label: "Hospital Status", icon: Building2 },
    { to: "/hospital/queue", label: "Emergency Queue", icon: ListOrdered },
  ];

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-purple-500 selection:text-white">
      <RoleHeader 
        roleTitle="HOSPITAL ER CLINICAL PORTAL" 
        roleBadgeColor="text-purple-400 bg-purple-500/10 border-purple-500/30" 
      />
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col md:flex-row gap-6">
        ...
        <aside className="w-full md:w-64 shrink-0 space-y-4">
          ...
          <div className="bg-gradient-to-br from-purple-950/60 to-slate-900 border border-purple-500/40 rounded-2xl p-4 shadow-xl space-y-2">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-purple-600/20 border border-purple-500/40 flex items-center justify-center text-purple-400">
                <HospitalIcon className="w-5 h-5" />
              </div>
              <div>
                <p className="text-xs font-bold text-white leading-tight">Emergency Department</p>
                <p className="text-[11px] text-purple-400 font-mono font-medium">Triage Desk Active</p>
              </div>
            </div>
          </div>
